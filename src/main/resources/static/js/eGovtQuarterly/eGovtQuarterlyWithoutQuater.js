...
function qGovProgressCtrlImp($scope, $http) {
    ...
    function save() {
        ...
        if(validate_data()){
        	var length=$scope.formObj.expenditures.length;
        	$scope.flag=true;
        	for(var i=0;i<length;i++)
        		{
        		console.log("data "+$scope.formObj.expenditures[i].postFilled+"   "+$scope.formObj.expenditures[i].incurred);
        		if($scope.formObj.expenditures[i].postFilled!='' && $scope.formObj.expenditures[i].postFilled!=null && $scope.formObj.expenditures[i].postFilled!=undefined && $scope.formObj.expenditures[i].postFilled>0)
    			{
    			if($scope.formObj.expenditures[i].incurred=='' || $scope.formObj.expenditures[i].incurred==null || $scope.formObj.expenditures[i].incurred==0)
    			{
    				toastr.error("Row No. "+(i+1)+" fill expenditure");
    				$scope.flag=false;
    			}
    		}
        		}
        	...
        	if($scope.flag)
        		...
        		{
        		toastr.error("Please fill expenditure");
        		}
        }
