...
<%@include file="qprCapacityBuildingJsWithoutQuarter.jsp"%>
<style>
.padding_left_local {
...
{
	var length=$("#training_activity_count_length").val();
	var flag=true;
	for(var i=0;i<length;i++)
		...
		if(($("#noOfUnitCompleted_"+i).val()!='' && $("#noOfUnitCompleted_"+i).val()!=null && $("#noOfUnitCompleted_"+i).val()!='undefined' && $("#noOfUnitCompleted_"+i).val()>0) 
				|| ($("#noOfDaysCompleted_"+i).val()!='' && $("#noOfDaysCompleted_"+i).val()!=null && $("#noOfDaysCompleted_"+i).val()!='undefined' && $("#noOfDaysCompleted_"+i).val()>0))
			{
	...
	if(flag){
		$('#origin').val(msg);
			...
			document.qpqCapacityBuilding.action = "qprCapacityBuilding.html?<csrf:token uri='qprCapacityBuilding.html'/>";
				...
				<c:set var="count_length" value="0"/>
					...
					<c:set var="count_length" value="${index.index+1}"/>
						...
						<!-- ends here -->
							...
							<c:choose>
								<c:when test="${not empty QPR_CB_ACT_DATA}">
									...
									<c:otherwise>
										...
										<input type="hidden" id="training_activity_count_length" value="${count_length}"/>
										<tr>
							...
							<c:choose>
								<c:when test="${QPR_CB_ACT_DATA.isFreeze}">
								<input type="submit" name="origin" value="SAVE"  class="btn bg-green waves-effect" disabled="disabled" />
								<button type="button" onclick="FreezeAndUnfreeze('unfreeze')"  class="btn bg-orange waves-effect" id="unfreezebtn">UNFREEZE</button>
								</c:when>
								<c:otherwise>
								<button type="button" name="origin" onclick="saveData('save')" class="btn bg-green waves-effect"  id="savebtn">SAVE</button>
								<c:choose>
