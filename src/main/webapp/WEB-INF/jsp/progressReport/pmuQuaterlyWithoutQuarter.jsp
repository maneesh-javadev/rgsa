...
function saveAndGetDataQtrRprt(msg){
	var length=$("#pmu_count_length").val();
	var flag=true;
	for(var i=0;i<length;i++)
		...
		if($("#noOfUnitCompleted_"+i).val()!='' && $("#noOfUnitCompleted_"+i).val()!=null && $("#noOfUnitCompleted_"+i).val()!='undefined' && $("#noOfUnitCompleted_"+i).val()>0)
			...
			if($("#expenditureIncurred_"+i).val()=='' || $("#expenditureIncurred_"+i).val()==null || $("#expenditureIncurred_"+i).val()==0)
				{
				alert("Row No. "+(i+1)+ " expenditure fill");
				flag=false;
				}
			}
		}
...
function validateWithCorrespondingFund(index){
	...
	if($('#expenditureIncurred_'+index).val() > tota_fund_cec){
		alert('total expenditure should not exceed '+ tota_fund_cec);
								...
								value="<csrf:token-value uri="pmuProgresReport.html" />" />
									...
									<table class="table table-bordered">
										...
										<c:set var="count_length" value="0"/>
											...
											<input type="hidden" id="pmu_count_length" value="${count_length}"/>
												<tr>
									...
									<div class="text-right">
										<form:button onclick="saveAndGetDataQtrRprt('save')" id="pmu_save"
