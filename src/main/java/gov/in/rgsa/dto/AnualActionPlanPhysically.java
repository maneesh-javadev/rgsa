...
@NamedNativeQuery( name="FETCH_ANUAL_ACTION_PLAN_DETAILS_PP" ,query="select ppd.id as id, ppo.pesa_post_name as column1 , CAST(ppd.no_of_units as varchar) as column2, CAST(ppd.unit_cost_per_month as varchar) as column3, CAST(COALESCE(ppd.no_of_months,'0') as varchar) as column4, CAST(COALESCE(ppd.funds,'0') as varchar) as column5, CAST(COALESCE(pp.additional_requirement ,'0')  as varchar)  as column6 , CAST(pp.user_type  as varchar) as column7, COALESCE(ppd.remarks,'N/A') as column8 , CAST(null as varchar) as column9  ,CAST(null as varchar) as column10 ,  CAST(6 as varchar)  as column11 ,CAST(null as varchar)  as column12 ,CAST(null as varchar) as column13 ,CAST(null as varchar)  as column14 from rgsa.pesa_plan pp ,  rgsa.pesa_plan_details ppd ,rgsa.pesa_post ppo where pp.pesa_plan_id =ppd.pesa_plan_id and  pp.state_code =:stateCode and pp.year_id=:yearId and ppd.designation_id =ppo.pesa_post_id and pp.user_type= 'C' and  pp.version_no=:version order by ppd.id",resultClass=AnualActionPlanPhysically.class),
...
@NamedNativeQuery( name="FETCH_ANUAL_ACTION_PLAN_DETAILS_PMU" ,query="\r\n" + 
		"select pad.pmu_activity_details_id id , pt.pmu_type_name column1, pat.pmu_activity_type_name column2 ,cast(pad.no_of_units as varchar )  column3,cast(pad.unit_cost as varchar) column4,cast(pad.no_of_months as varchar) column5,cast(pad.other_expenses as varchar) column6, case when pad.remarks = '' then 'N/A' ELSE  pad.remarks end  column7,CAST(COALESCE(pa.additional_requirement ,'0')  as varchar) as column8,CAST(pad.funds as varchar)  as column9 ,CAST(null as varchar) as column10 ,  CAST(12 as varchar)  as column11 ,CAST(null as varchar)  as column12 ,CAST(null as varchar) as column13, CAST(null as varchar)  as column14  from rgsa.pmu_activity pa , rgsa.pmu_activity_details pad , rgsa.pmu_activity_type pat,rgsa.pmu_type pt where state_code=:stateCode and year_id=:yearId and pa.pmu_activity_id =pad.pmu_activity_id and pad.pmu_activity_type_id =pat.pmu_activity_type_id and pat.pmu_type_id =pt.pmu_type_id and pa.version_no=:version and user_type='C' and pt.pmu_type_id=1",resultClass=AnualActionPlanPhysically.class),
