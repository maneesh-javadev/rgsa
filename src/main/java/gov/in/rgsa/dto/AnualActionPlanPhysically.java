...
@NamedNativeQuery( name="FETCH_ANUAL_ACTION_PLAN_DETAILS_PB" ,query="select pbad.id as id ,ga.activity_name column1, cast(pba.additional_requirement as varchar) as  column2, cast(COALESCE(pbad.unit_cost,'0') as varchar) as column3, cast(COALESCE(pbad.funds,'0') as varchar ) as column4,cast(pbad.aspirational_gps as varchar) as column5,cast(mission_antyodaya_gps as varchar) as  column6, cast(COALESCE(pbad.no_of_gps,'0') as varchar) as  column7, cast(pbad.work_type as varchar )as column8,pbad.remarks as column9 ,CAST(null as varchar) as column10 ,  CAST(3 as varchar)  as column11 ,CAST(null as varchar)  as column12 ,CAST(null as varchar) as column13 ,CAST(null as varchar)  as column14 from rgsa.panhcayat_bhawan_activity pba ,rgsa.panhcayat_bhawan_activity_details  pbad ,rgsa.gps_activity ga where pba.panhcayat_bhawan_activity_id =pbad.panhcayat_bhawan_activity_id and ga.activity_id =pbad.activity_id and  pba.state_code =:stateCode and pba.year_id=:yearId and pba.version_no =:version and user_type='S' order by id",resultClass=AnualActionPlanPhysically.class),
